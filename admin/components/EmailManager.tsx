...
export const EmailManager: React.FC<EmailManagerProps> = ({ order, onClose }) => {
  ...
  useEffect(() => {
    ...
    const loadCustomTemplates = () => {
      console.log('Cargando plantillas personalizadas...');
      const savedTemplates = localStorage.getItem('emailTemplates');
      console.log('Plantillas encontradas:', savedTemplates);
      
      if (savedTemplates) {
        try {
          const templates = JSON.parse(savedTemplates);
          console.log('Plantillas parseadas:', templates);
          ...
          console.error('Error al parsear plantillas:', error);
        }
      } else {
        console.log('No se encontraron plantillas guardadas, creando plantillas por defecto...');
  ...
  const handleSendEmail = async (type: 'customer' | 'admin') => {
    ...
    try {
      ...
      console.log(`Email ${type} enviado a ${recipient}`);
  ...
  const handleSendCustomEmail = async (template: EmailTemplate) => {
    ...
    try {
      ...
      console.log(`Email ${template.type} enviado a ${recipient}`);
        ...
        <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; color: #666;">
            ...
            <p style="font-size: 12px; margin-top: 10px;">
                Este es un correo automatico, por favor no responda a esta direccion.
                        ...
                        console.log('Creando plantillas por defecto...', defaultTemplates);
                        localStorage.setItem('emailTemplates', JSON.stringify(defaultTemplates));
                        setCustomTemplates(defaultTemplates);
